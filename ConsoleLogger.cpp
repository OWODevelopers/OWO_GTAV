							...
							const char	*helper_executable/*=NULL*/)
{
	...
	{	// no name was give , create name based on the current address+time
		...
		sprintf_s((char*)logger_name, 120,"logger%d_%lu",(int)this,now);
	}
	else
	{	// just use the given name
		strcat_s(m_name,logger_name);
	}
	...
	if (m_hPipe==INVALID_HANDLE_VALUE)
	{	// failure
		MessageBox(NULL,(LPCSTR)"CreateNamedPipe failed", (LPCSTR)"ConsoleLogger failed",MB_OK);
		return -1;
	}
	...
	sprintf_s(cmdline, MAX_PATH,"%s/FiveM/FiveM.app/plugins/ConsoleLoggerHelper.exe %s", pathBuffer,logger_name);
	BOOL bRet = CreateProcessA(NULL,cmdline,NULL,NULL,FALSE,CREATE_NEW_CONSOLE,NULL,NULL,&si,&pi);
	if (!bRet)
	{	// on failure - try to get the path from the environmentFOLDERID_LocalAppData
			MessageBox(NULL, (LPCSTR)("Helper executable not found %s", cmdline), (LPCSTR)"ConsoleLogger failed",MB_OK);
			CloseHandle(m_hPipe);
			m_hPipe = INVALID_HANDLE_VALUE;
			return -1;
	}
	...
	if (!bConnected)
	{
		MessageBox(NULL, (LPCSTR)"ConnectNamedPipe failed", (LPCSTR)"ConsoleLogger failed",MB_OK);
	...
	sprintf_s(buffer, 128,"TITLE: %s\r\n",lpszWindowTitle);
	WriteFile(m_hPipe,buffer,strlen(buffer),&cbWritten,NULL);
	if (cbWritten!=strlen(buffer))
	{
		MessageBox(NULL, (LPCSTR)"WriteFile failed(1)", (LPCSTR)"ConsoleLogger failed",MB_OK);
		DisconnectNamedPipe(m_hPipe);
		CloseHandle(m_hPipe);
		m_hPipe=INVALID_HANDLE_VALUE;
		return -1;
	...
	{	// Send buffer-size
		sprintf_s(buffer, 128,"BUFFER-SIZE: %dx%d\r\n",buffer_size_x,buffer_size_y);
		WriteFile(m_hPipe,buffer,strlen(buffer),&cbWritten,NULL);
		if (cbWritten!=strlen(buffer))
		{
			MessageBox(NULL, (LPCSTR)"WriteFile failed(2)", (LPCSTR)"ConsoleLogger failed",MB_OK);
			DisconnectNamedPipe(m_hPipe);
			CloseHandle(m_hPipe);
			m_hPipe=INVALID_HANDLE_VALUE;
			return -1;
	...
	if (cbWritten!=1)
	{
		MessageBox(NULL, (LPCSTR)"WriteFile failed(3)", (LPCSTR)"ConsoleLogger failed",MB_OK);
		DisconnectNamedPipe(m_hPipe);
		CloseHandle(m_hPipe);
		m_hPipe=INVALID_HANDLE_VALUE;
		return -1;
